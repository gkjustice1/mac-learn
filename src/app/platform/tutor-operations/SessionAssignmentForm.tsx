...
function Submit({ disabled, pending }: { disabled: boolean; pending: boolean }) {
  return (
    ...
    >
      {pending ? "Scheduling..." : "Assign session"}
    </button>
  );
}
...
export function SessionAssignmentForm({
  ...
  const [state, action, pending] = useActionState(scheduleTutorSession, {
    error: null,
    scheduled: false,
  });
  ...
  return (
    <form
      action={submitWithBrowserTimeZone}
    ...
    >
      ...
      {students.length === 0 && (
        ...
        >
          No active students are available. Create and link a student before
      ...
      <label className="grid gap-2 text-sm font-semibold">
        ...
        <select
          ...
          className="rounded-xl border p-3 font-normal"
        >
          <option value="">Select a compatible Tutor</option>
          {compatibleTutors.map((x) => (
            <option key={x.id} value={x.id}>
              {x.label}
            </option>
          ))}
      ...
      <label className="grid gap-2 text-sm font-semibold">
        Subject (optional)
        <select name="subject_id" className="rounded-xl border p-3 font-normal">
          <option value="">General tutoring</option>
          {subjects.map((x) => (
            <option key={x.id} value={x.id}>
              {x.label}
            </option>
      ...
      <Submit disabled={unavailable} pending={pending} />
    </form>
  );
}
