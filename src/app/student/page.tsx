...
export default async function StudentPage() {
  ...
  const [studentsResult2, sessionsResult, assignmentsResult, contentResult, feedbackResult, progressResult] = await Promise.all([
    studentIds.length ? supabase.from("students").select("id, first_name, last_name, grade_level, school_name").in("id", studentIds) : Promise.resolve({ data: [], error: null }),
  ...
  const failure = [studentsResult2, sessionsResult, assignmentsResult, contentResult, feedbackResult, progressResult].find((result) => result.error);
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950">
    ...
    <div className="mx-auto grid max-w-7xl gap-8 px-6 py-8">
      ...
      <section id="sessions"><h2 className="text-xl font-bold">Sessions</h2><div className="mt-4 grid gap-3">{sessions.length ? sessions.map((session) => <article key={session.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><p className="font-semibold">{session.subject?.[0]?.name ?? "General tutoring"}</p><p className="mt-2 text-sm text-slate-600">{formatDateTime(session.start_time)} · <span className="capitalize">{session.status?.replaceAll("_", " ")}</span></p>{session.zoom_link ? <a href={session.zoom_link} className="mt-3 inline-block font-semibold text-lime-700 underline">Join session</a> : null}</article>) : <EmptyState>No sessions are scheduled.</EmptyState>}</div></section>
