...
export default async function StudentPage() {
  ...
  const [sessionsResult, assignmentsResult, contentResult, feedbackResult, progressResult, organizationsResult, configurationsResult, sitesResult] = await Promise.all([
    ...
    organizationIds.length ? supabase.from("sites").select("id, organization_id, name, timezone").in("organization_id", organizationIds) : Promise.resolve({ data: [], error: null }),
  ]);
  const failure = [sessionsResult, assignmentsResult, contentResult, feedbackResult, progressResult, organizationsResult, configurationsResult, sitesResult].find((result) => result.error);
  ...
  const scopeLabelForStudent = (studentId: string) => {
    const student = studentScopes.get(studentId);
    const organizationName = student?.organization_id ? organizationNames.get(student.organization_id) : undefined;
    const siteName = student?.primary_site_id ? siteNames.get(student.primary_site_id) : undefined;
    return [organizationName ?? "Assigned organization", siteName].filter(Boolean).join(" · ");
  };
