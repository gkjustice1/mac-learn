...
as $$
  select not exists (
    ...
    where enterprise_user.id = auth.uid()
      and (
        enterprise_user.account_status <> 'active'
        or exists (
          select 1
          from public.guardians guardian
          where guardian.person_id = enterprise_user.person_id
            and (
              requested_organization_id is null
              or guardian.organization_id = requested_organization_id
            )
        )
      )
  );
...
revoke all on function public.mac_can_use_legacy_family_link(uuid) from public;
...
using (public.mac_is_organization_admin(organization_id) or exists (
  select 1 from public.users enterprise_user
...
on public.guardian_student_relationships for select to authenticated
using (public.mac_is_organization_admin(organization_id) or exists (
  ...
  where guardian.id = guardian_student_relationships.guardian_id
    ...
    and (guardian_student_relationships.valid_from is null or guardian_student_relationships.valid_from <= current_date)
    and (guardian_student_relationships.valid_until is null or guardian_student_relationships.valid_until >= current_date)
));
...
using (
  exists (
    select 1 from public.profiles legacy_parent
  ...
  or exists (
    select 1 from public.guardian_student_relationships relationship
    join public.guardians guardian on guardian.id = relationship.guardian_id and guardian.organization_id = relationship.organization_id
    join public.users enterprise_user on enterprise_user.person_id = guardian.person_id
    where relationship.student_id = students.id
      ...
      and enterprise_user.id = (select auth.uid())
      and enterprise_user.account_status = 'active'
      and (relationship.valid_from is null or relationship.valid_from <= current_date)
      and (relationship.valid_until is null or relationship.valid_until >= current_date)
  )
);

comment on policy "Authenticated guardians view their own guardian records" on public.guardians is
  'A guardian can view only their own enterprise guardian record; organization and platform admins retain management access.';
comment on policy "Authenticated guardians view active educational relationships" on public.guardian_student_relationships is
  'A guardian can view only active, currently valid relationships that grant educational access.';
comment on policy "Authenticated families view only related students" on public.students is
