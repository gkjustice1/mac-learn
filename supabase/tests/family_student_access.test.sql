...
select is((select count(*) from public.students),0::bigint,'an unmigrated legacy administrator has no student access');
select is((select count(*) from public.sessions),0::bigint,'unmigrated legacy administrator has no session access');
reset role;
update public.profiles set role='parent' where user_id='14000000-0000-4000-8000-000000000002';
update public.students set parent_id='64000000-0000-4000-8000-000000000002' where id='74000000-0000-4000-8000-000000000003';
set local role authenticated;
select is((select count(*) from public.students),0::bigint,'owning legacy parent link alone grants no student access');
reset role;
update public.profiles set role='admin' where user_id='14000000-0000-4000-8000-000000000002';
set local role authenticated;
...
reset role;
select is(to_regprocedure('public.mac_can_use_legacy_admin_access()'),null::regprocedure,'legacy administration helper is retired');
select is(to_regprocedure('public.mac_can_use_legacy_family_link(uuid)'),null::regprocedure,'legacy family helper is retired');
select * from finish();
rollback;
