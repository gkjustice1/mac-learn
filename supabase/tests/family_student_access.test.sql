...
values ('24000000-0000-4000-8000-000000000001', 'Family Access Test Organization', 'family-access-test');
...
values ('14000000-0000-4000-8000-000000000001', '34000000-0000-4000-8000-000000000001', 'active');
...
insert into public.students (
  id, parent_id, first_name, last_name, grade_level, organization_id, person_id
) values
  ...
  ('74000000-0000-4000-8000-000000000003', '64000000-0000-4000-8000-000000000001', 'Restricted', 'Student', '5', '24000000-0000-4000-8000-000000000001', '34000000-0000-4000-8000-000000000004');

insert into public.guardian_student_relationships (
  organization_id, guardian_id, student_id, relationship_type, educational_access, valid_from, valid_until
) values
  ('24000000-0000-4000-8000-000000000001', '44000000-0000-4000-8000-000000000001', '74000000-0000-4000-8000-000000000001', 'parent', true, current_date - 1, current_date + 1),
  ('24000000-0000-4000-8000-000000000001', '44000000-0000-4000-8000-000000000001', '74000000-0000-4000-8000-000000000003', 'parent', false, current_date - 1, current_date + 1);

select ok(
  has_table_privilege('authenticated', 'public.students', 'select')
  and has_table_privilege('authenticated', 'public.guardians', 'select')
  and has_table_privilege('authenticated', 'public.guardian_student_relationships', 'select'),
  'authenticated users retain table SELECT privileges while RLS controls family rows'
);
...
select is((select count(*) from public.guardians), 1::bigint,
  'a guardian can view only their own guardian record');
select is((select count(*) from public.guardian_student_relationships), 1::bigint,
  'a guardian can view only active educational relationships');
select is((select count(*) from public.students), 1::bigint,
  'a guardian can view a student linked by an active educational relationship');
select is((select first_name from public.students where id = '74000000-0000-4000-8000-000000000001'),
  'Related', 'the related student remains visible to the guardian');
...
select throws_ok(
  $$insert into public.guardian_student_relationships (organization_id, guardian_id, student_id, relationship_type)
    values ('24000000-0000-4000-8000-000000000001', '44000000-0000-4000-8000-000000000001', '74000000-0000-4000-8000-000000000002', 'parent')$$,
  '42501',
  'new row violates row-level security policy for table "guardian_student_relationships"',
  'a guardian cannot create a relationship through the Data API'
);
