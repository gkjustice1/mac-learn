...
-- Test-only transactional grant: production intentionally does not grant
-- authenticated UPDATE on students. This suite exercises student UPDATE RLS
-- and enrollment audit triggers, so grant only that privilege in this
-- rollback-bound test transaction.
...
select ok(has_function_privilege('authenticated','public.mac_admin_enroll_student(text,text,text,text,uuid,uuid,date,text,uuid,text)','EXECUTE'),'authenticated may call enrollment function');
select ok(not has_function_privilege('anon','public.mac_admin_enroll_student(text,text,text,text,uuid,uuid,date,text,uuid,text)','EXECUTE'),'anon cannot call enrollment function');
select ok(not (select attnotnull from pg_catalog.pg_attribute where attrelid='public.students'::regclass and attname='parent_id'),'legacy student parent profile reference is nullable');
select is((select constraint_definition.confdeltype::text from pg_catalog.pg_constraint constraint_definition where constraint_definition.conrelid='public.students'::regclass and constraint_definition.conname='students_parent_id_fkey'),'n','deleting a guardian profile clears only the legacy reference');
select ok((select qual ilike '%mac_relationship_calendar_date%' from pg_catalog.pg_policies where schemaname='public' and tablename='guardian_student_relationships' and policyname='Authenticated guardians view active educational relationships'),'guardian relationship visibility uses the student site timezone');
select ok((select qual ilike '%mac_family_can_access_student%' from pg_catalog.pg_policies where schemaname='public' and tablename='students' and policyname='Authenticated families view only related students'),'family student visibility uses the active role-aware access helper');
select ok(has_function_privilege('authenticated','public.mac_admin_search_guardians(uuid,uuid,text)','EXECUTE'),'authenticated users may call the authorized guardian search function');
select ok(not has_function_privilege('anon','public.mac_admin_search_guardians(uuid,uuid,text)','EXECUTE'),'anonymous users cannot call the guardian search function');

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select is((select count(*) from public.mac_admin_search_guardians('b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001','Guardian Alpha')),1::bigint,'platform administrator can search an enrollable guardian through the scoped RPC');
reset role;

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select lives_ok($$select public.mac_admin_enroll_student('Real','Learner','Grade 4','Example Elementary','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000004','parent_guardian')$$,'platform admin enrolls a student with an invited guardian');
reset role;
...
select is((select constraint_definition.confdeltype::text from pg_catalog.pg_constraint constraint_definition where constraint_definition.conrelid='public.student_enrollment_events'::regclass and constraint_definition.contype='f' and constraint_definition.conkey=array[(select attnum from pg_catalog.pg_attribute where attrelid='public.student_enrollment_events'::regclass and attname='student_id')]::smallint[]),'r','student deletion is restricted to preserve enrollment audit history');
...
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
update public.students set enterprise_status='withdrawn' where first_name='Real' and last_name='Learner';
reset role;
...
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select throws_ok($$select public.mac_admin_enroll_student('Restricted','Guardian','Grade 4','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000004','guardian')$$,'P0001','The selected guardian is not active and must be reactivated separately','restricted guardian cannot be used for a new enrollment');
reset role;
...
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select is((select count(*) from public.mac_admin_search_guardians('b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001','Guardian Alpha')),0::bigint,'restricted guardian is excluded from search results');
reset role;
update public.guardians set status='active' where person_id='d1000000-0000-4000-8000-000000000004';

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000002","role":"authenticated"}',true);
select throws_ok($$select public.mac_admin_enroll_student('Cross','Tenant','Grade 5','','b1000000-0000-4000-8000-000000000002','c1000000-0000-4000-8000-000000000002',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000005','guardian')$$,'P0001','Not authorized to enroll students in this organization','organization admin cannot enroll across tenants');
reset role;

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select throws_ok($$select public.mac_admin_enroll_student('Wrong','Site','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000002',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000004','guardian')$$,'P0001','The selected site is not active in this organization','cross-tenant site is rejected');
select throws_ok($$select public.mac_admin_enroll_student('Wrong','Guardian','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000005','guardian')$$,'P0001','The selected guardian is not invited or active in this organization and site','cross-tenant guardian is rejected');
reset role;

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000006","role":"authenticated"}',true);
select throws_ok($$select public.mac_admin_enroll_student('Unauthorized','Student','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000004','guardian')$$,'P0001','Not authorized to enroll students in this organization','Tutor cannot enroll students');
select throws_ok($$insert into public.student_enrollment_events (student_id,organization_id,site_id,guardian_id,event_type) select student.id,student.organization_id,student.primary_site_id,relationship.guardian_id,'updated' from public.students student join public.guardian_student_relationships relationship on relationship.student_id=student.id where student.first_name='Real'$$,'42501',null,'authenticated users cannot write audit rows directly');
reset role;

set local role authenticated;
select set_config('request.jwt.claims','{"sub":"a1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select throws_ok($$select public.mac_admin_enroll_student('Invalid','Status','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,'withdrawn','a1000000-0000-4000-8000-000000000004','guardian')$$,'P0001','New enrollment status must be active or inactive','unsupported initial status is rejected');
select throws_ok($$select public.mac_admin_enroll_student('Null','Status','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,null,'a1000000-0000-4000-8000-000000000004','guardian')$$,'P0001','New enrollment status must be active or inactive','null initial status is rejected');
select throws_ok($$select public.mac_admin_enroll_student('Invalid','Relationship','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date,'active','a1000000-0000-4000-8000-000000000004','unsupported')$$,'P0001','Guardian relationship type is invalid','unsupported guardian relationship is rejected');
select throws_ok($$select public.mac_admin_enroll_student('Future','Inactive','Grade 5','','b1000000-0000-4000-8000-000000000001','c1000000-0000-4000-8000-000000000001',(now() at time zone 'America/New_York')::date+1,'inactive','a1000000-0000-4000-8000-000000000004','guardian')$$,'P0001','Enrollment start date cannot be in the future','future enrollment is rejected until scheduling activation exists');
reset role;
